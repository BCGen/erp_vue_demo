<template>
  <div>
    <app-form ref="form" v-on="$listeners">
      ...
      <form-group
        class="col-1212 col-xl-8"
        input-class="col-8 col-sm-9 col-lg-10 pr-lg-11"
        label="104 網址"
        label-for="inputUrl"
      ...
      <form-group
        class="col-12 col-lg-6 col-xl-4"
        input-class="col-6 col-sm-7 col-xl-5"
        suffix-class="col-6 col-sm-5 col-xl-7"
        label="排序"
      >
        <input
          id="inputNature"
      ...
      <form-group
        ...
        input-class="col-8 col-sm-9 col-lg-10 col-xl-8"
        label="上班地點"
      ...
      <form-group
        class="col-12 col-xl-8 pr-lg-11"
        label="工作內容"
      ...
      <form-group class="col-12" label="語文條件">
        <div
          ...
          class="d-inline"
        >
          <input-checkbox v-model="formData.lang" :label="value" :value="id" />
          <br v-if="index % 3 === 2" />
        </div>
      </form-group>
      ...
      <form-group class="col-12" label="擅長工具">
        <div v-for="{ id, value } in skillOptions" :key="id" class="d-inline">
          <input-checkbox v-model="formData.skill" :label="value" :value="id" />
        </div>
      </form-group>
...
export default {
  ...
  computed: {
    workSpace() {
      const { wpid } = this.formData

      if (Array.isArray(wpid) && wpid.length > 0) {
        return wpid
          .map(itemId => {
            const option = this.workspaceOptions.find(({ id }) => id == itemId)
